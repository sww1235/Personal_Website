...
18. Install Lenovo Vantage from Lenovo's website and make sure BIOS and drivers
	are installed and up to date. (Especially important for Thunderbolt
	devices)
...
12. Type `bcdedit /set "{bootmgr}" description "rEFInd description"` to set a
	...
	Now, when the laptop is rebooted, rEFInd should present itself with an
	option to boot into windows 10.
...
1.  Login to newly installed Void Linux using root account and password found
...
14. Set up ssh-agent using user specific services. Instructions taken from
	...
	7.  Create run script for `ssh-agent` service by adding the following into
		`~/runit/sv/ssh-agent/run`.
